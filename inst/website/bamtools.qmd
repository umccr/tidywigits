...
```{r}
#| label: bamtools_genecvg
tbl_nm <- "bamtools_genecvg"
d1 <- dat$data |>
  unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "genes")
d2 <- dat$data |>
  unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "cvg")
```
...
```{r}
#| label: cvg_gene_plot_bar
bt_utils$cvg_gene_plot_bar(d2$res)
```
