...
CANCER_TYPE_METADATA <- (function(x) {
...
format_numbers <- function(values, configs) {
  for (config in configs) {
    if (!all(c("lower", "upper", "func") %in% names(config))) {
      stop(
        "`configs` must be list containing items in the format: `list(lower={numeric}, upper={numeric}, func={function})`"
      )
    }
  }
...
get_plot_data_probs <- function(vis_data) {
  ...
  plot_data_supertypes <- lapply(plot_data_split, function(df) {
    ...
    df$data_label_nudge_x <- if (nrow(df) %% 2 == 0) {
      0.5
    } else {
      0
    }
...
plot_heatmap <- function(
  ...
  get_plot_data_first_row_hline <- function(plot_data) {
    ...
    plot_data_hline <- plot_data_hline[
      !duplicated(interaction(plot_data_hline$cancer_type, plot_data_hline$row_label)),
    ]
  ...
  if (show_first_row_hline) {
    ...
    p <- p +
      geom_hline(
        data = plot_data_hline,
        mapping = aes(yintercept = yintercept - 0.5),
        linetype = "A1"
      )
  }
...
plot_probs <- function(vis_data) {
  ...
  if (snv_count < LOW_SNV_COUNT_THRES) {
    warning_string <- sprintf(
      " | WARNING: Sample has <%s SNVs. Predictions may be unreliable",
      LOW_SNV_COUNT_THRES
    )
  } else {
    warning_string <- ""
...
plot_signatures <- function(vis_data) {
  ...
  plot_data$data_label <- format_numbers(
    plot_data$data_value,
    NUMBER_FORMATTING_SIGNATURE_QUANTILES
  )
  ...
  quantile_info <- list(
    ...
    labels = c(
      ...
      "0",
      ">0 - 0.95 (in expected range)",
      "0.95 - 1.2 (above expected range)",
      ">1.2 (well above expected range)"
    ),
...
plot_one_sample <- function(VIS_DATA, sample_id) {
  ...
  ## Combine --------------------------------
  ## Heights
  heights <- sapply(plots, function(p) {
    p$nrows
  })
  heights <- heights[heights > 0] ## Remove 0 height plots

  ## Widths
  widths <- sapply(plots, function(p) {
    p$ncols
  })
...
write_plots <- function(VIS_DATA, PLOT_PATH) {
  ...
  if (grepl(".pdf$", PLOT_PATH)) {
    ...
    if (length(sample_ids) > 1) {
      stop("Plot output file extension must be .pdf for multi-sample plotting")
    }
